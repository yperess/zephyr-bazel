...
#define ZTEST TEST
#define zassert_equal(x,y,z) EXPECT_EQ((x),(y))
#define zassert_true(x,y) EXPECT_TRUE((x))
#define zassert_false(x,y) EXPECT_FALSE((x))
#define zassert_null(x,y) EXPECT_EQ((x), nullptr)
#define zassert_mem_equal(buf, exp, size) EXPECT_FALSE(memcmp(buf, exp, size))
#define zassert_between_inclusive(x, min, max, msg) EXPECT_TRUE((x) >= (min) && (x) <= (max))
#define zassert_within(a,b,d,log) EXPECT_TRUE((((a) >= ((b) - (d))) && ((a) <= ((b) + (d)))))

namespace {

TEST(DevicetreeApiTest, TrivialIncludeSuccess) {
  EXPECT_TRUE(true);
}

ZTEST(DevicetreeApi, TestPathProps)
{
...
ZTEST(DevicetreeApi, TestAliasProps)
{
...
ZTEST(DevicetreeApi, test_nodelabel_props)
{
...
ZTEST(DevicetreeApi, test_inst_props)
{
	/*
	 * Careful:
	 *
...
ZTEST(DevicetreeApi, test_any_inst_prop)
{
...
ZTEST(DevicetreeApi, test_default_prop_access)
{
	/*
...
ZTEST(DevicetreeApi, test_has_path)
{
...
ZTEST(DevicetreeApi, test_has_nodelabel)
{
...
ZTEST(DevicetreeApi, test_has_compat)
{
	int compats;
...
ZTEST(DevicetreeApi, test_has_status)
{
...
#undef DT_DRV_COMPAT
	 ...
	 */
	zassert_equal(DT_HAS_COMPAT_ON_BUS_STATUS_OKAY(vnd_spi_device, spi), 1, NULL);
	zassert_equal(DT_HAS_COMPAT_ON_BUS_STATUS_OKAY(vnd_spi_device, i2c), 0, NULL);

	zassert_equal(DT_HAS_COMPAT_ON_BUS_STATUS_OKAY(vnd_i2c_device, i2c), 1, NULL);
	zassert_equal(DT_HAS_COMPAT_ON_BUS_STATUS_OKAY(vnd_i2c_device, spi), 0, NULL);
...
ZTEST(DevicetreeApi, test_vendor)
{
  //TODO(sergiosoares): fix test_vendor test. DT_NODE_VENDOR_HAS_IDX doesn't work for multiple compatible names.
...
ZTEST(DevicetreeApi, test_model)
{
  // // TODO(sergiosoares): fix test_model test. DT_NODE_MODEL_HAS_IDX doesn't work for multiple compatible names.
...
ZTEST(DevicetreeApi, test_reg_64)
{
	/* DT_REG_ADDR_U64 */
...
ZTEST(DevicetreeApi, test_irq_level)
{
	/* DT_IRQ_LEVEL */
...
ZTEST(DevicetreeApi, test_phandles)
{
	...
	zassert_equal(ARRAY_SIZE(phs_freqs), (size_t)2, "");
	...
	zassert_equal(phs_freqs[0], (size_t)100000, "");
	zassert_equal(phs_freqs[1], (size_t)2000000, "");
	...
	zassert_equal(ARRAY_SIZE(gps), (size_t)2, "");
	...
	zassert_equal(gps[0].flags, (size_t)20, "");

	zassert_equal(gps[1].pin, 30, "");
	zassert_equal(gps[1].flags, (size_t)40, "");
...
#undef DT_DRV_COMPAT
#define DT_DRV_COMPAT vnd_adc_temp_sensor
ZTEST(DevicetreeApi, test_io_channels)
{
...
ZTEST(DevicetreeApi, test_macro_names)
{
	/* white box */
...
static const char *c[] = DT_PROP(TEST_ARRAYS, c);

ZTEST(DevicetreeApi, test_arrays)
{
	int ok;

	zassert_equal(ARRAY_SIZE(a), (size_t)3, "");
	zassert_equal(ARRAY_SIZE(b), (size_t)4, "");
	zassert_equal(ARRAY_SIZE(c), (size_t)2, "");
...
ZTEST(DevicetreeApi, test_foreach)
{
	/*
...
#undef DT_DRV_COMPAT
#define DT_DRV_COMPAT vnd_gpio_device
ZTEST(DevicetreeApi, test_foreach_status_okay)
{
	/*
...
ZTEST(DevicetreeApi, test_foreach_prop_elem)
{
...
#define BY_IDX_COMMA(node_id, prop, idx) DT_PROP_BY_IDX(node_id, prop, idx),
	...
	zassert_equal(ARRAY_SIZE(array_a), (size_t)3, "");
	...
	zassert_equal(ARRAY_SIZE(array_b), (size_t)4, "");
	...
	zassert_equal(ARRAY_SIZE(array_c), (size_t)2, "");
	...
	zassert_equal(ARRAY_SIZE(array_val), (size_t)1, "");
	...
	zassert_equal(ARRAY_SIZE(string_zephyr_user), (size_t)1, "");
...
#define PATH_COMMA(node_id, prop, idx) \
	...
	zassert_equal(ARRAY_SIZE(array_ph), (size_t)1, "");
	...
	zassert_equal(ARRAY_SIZE(array_zephyr_user_ph), (size_t)1, "");
	...
	zassert_equal(ARRAY_SIZE(array_phs), (size_t)2, "");
...
#define PIN_COMMA(node_id, prop, idx) DT_GPIO_PIN_BY_IDX(node_id, prop, idx),
	...
	zassert_equal(ARRAY_SIZE(array_gpios), (size_t)2, "");
...
#undef PATH_COMMA
	...
	zassert_equal(ARRAY_SIZE(array_sep), (size_t)3, "");
...
ZTEST(DevicetreeApi, test_foreach_prop_elem_varg)
{
#define TIMES_TWO_ADD(node_id, prop, idx, arg) \
	...
	zassert_equal(ARRAY_SIZE(array), (size_t)3, "");
...
// {
// 	struct test_gpio_data *data = (struct test_gpio_data *)dev->data;
...
// ZTEST(DevicetreeApi, test_devices)
// {
...
ZTEST(DevicetreeApi, test_cs_gpios)
{
...
ZTEST(DevicetreeApi, test_chosen)
{
...
ZTEST(DevicetreeApi, test_enums)
{
	enum {
		MY_ENUM_zero = 0xff,
		MY_ENUM_ZERO = 0xaa,
...
ZTEST(DevicetreeApi, test_enums_required_false)
{
...
ZTEST(DevicetreeApi, test_inst_enums)
{
#undef DT_DRV_COMPAT
...
#undef DT_DRV_COMPAT
#define DT_DRV_COMPAT vnd_adc_temp_sensor
ZTEST(DevicetreeApi, test_clocks)
{
...
#undef DT_DRV_COMPAT
#define DT_DRV_COMPAT vnd_spi_device
ZTEST(DevicetreeApi, test_parent)
{
...
ZTEST(DevicetreeApi, test_gparent)
{
...
ZTEST(DevicetreeApi, test_children)
{
...
#undef DT_DRV_COMPAT
#define DT_DRV_COMPAT vnd_child_bindings
ZTEST(DevicetreeApi, test_child_nodes_list)
{
	...
	zassert_equal(ARRAY_SIZE(vals), (size_t)3, "");
	zassert_equal(ARRAY_SIZE(vals_sep), (size_t)3, "");
	zassert_equal(ARRAY_SIZE(vals_inst), (size_t)3, "");
	zassert_equal(ARRAY_SIZE(vals_inst_sep), (size_t)3, "");
	zassert_equal(ARRAY_SIZE(vals_status_okay), (size_t)2, "");
	zassert_equal(ARRAY_SIZE(vals_status_okay_inst), (size_t)2, "");
	zassert_equal(ARRAY_SIZE(vals_status_okay_sep), (size_t)2, "");
	zassert_equal(ARRAY_SIZE(vals_status_okay_inst_sep), (size_t)2, "");
...
#undef DT_DRV_COMPAT
#define DT_DRV_COMPAT vnd_child_bindings
ZTEST(DevicetreeApi, test_child_nodes_list_varg)
{
...
ZTEST(DevicetreeApi, test_great_grandchild)
{
...
ZTEST(DevicetreeApi, test_ranges_pcie)
{
...
#define LENGTH(node_id, idx) DT_RANGES_LENGTH_BY_IDX(node_id, idx),
	...
	int count = DT_NUM_RANGES(TEST_RANGES_PCIE);
	...
	zassert_equal(ranges_pcie_flags[0], (uint64_t)0x1000000, "");
	zassert_equal(ranges_pcie_flags[1], (uint64_t)0x2000000, "");
	zassert_equal(ranges_pcie_flags[2], (uint64_t)0x3000000, "");
	zassert_equal(ranges_child_bus_addr[0], (uint64_t)0, "");
	zassert_equal(ranges_child_bus_addr[1], (uint64_t)0x10000000, "");
	zassert_equal(ranges_child_bus_addr[2], (uint64_t)0x8000000000, "");
	zassert_equal(ranges_parent_bus_addr[0], (uint64_t)0x3eff0000, "");
	zassert_equal(ranges_parent_bus_addr[1], (uint64_t)0x10000000, "");
	zassert_equal(ranges_parent_bus_addr[2], (uint64_t)0x8000000000, "");
	zassert_equal(ranges_length[0], (uint64_t)0x10000, "");
	zassert_equal(ranges_length[1], (uint64_t)0x2eff0000, "");
	zassert_equal(ranges_length[2], (uint64_t)0x8000000000, "");
...
ZTEST(DevicetreeApi, test_ranges_other)
{
...
#define LENGTH(node_id, idx) DT_RANGES_LENGTH_BY_IDX(node_id, idx),
	...
	uint64_t count = DT_NUM_RANGES(TEST_RANGES_OTHER);
	...
	zassert_equal(count, (uint64_t)2, "");
...
ZTEST(DevicetreeApi, test_ranges_empty)
{
...
ZTEST(DevicetreeApi, test_compat_get_any_status_okay)
{
	zassert_true(
		DT_SAME_NODE(
...
ZTEST(DevicetreeApi, test_dep_ord)
{
#define ORD_IN_ARRAY(ord, array) ord_in_array(ord, array, ARRAY_SIZE(array))
	...
	/* DT_DEP_ORD */
	zassert_equal(root_ord, (unsigned int)0, "");
	...
	zassert_equal(ARRAY_SIZE(root_requires), (size_t)0, "");
...
ZTEST(DevicetreeApi, test_node_name)
{
...
ZTEST(DevicetreeApi, test_node_child_idx)
{
...
// TODO(sergiosoares): this fail with 2652:1: error: mixture of designated and
// non-designated initializers in the same initializer list is a C99 extension [-Werror,-Wc99-designator]
...
ZTEST(DevicetreeApi, test_fixed_partitions)
{
...
#define FIXED_PARTITION_ID_COMMA(node_id) DT_FIXED_PARTITION_ID(node_id),
	...
	for (size_t i = 0; i < ARRAY_SIZE(ids); i++) {
		zassert_between_inclusive(ids[i], 0, (int)ARRAY_SIZE(ids) - 1, "");
...
ZTEST(DevicetreeApi, test_string_token)
{
#undef DT_DRV_COMPAT
...
ZTEST(DevicetreeApi, test_string_idx_token)
{
...
ZTEST(DevicetreeApi, test_string_unquoted)
{
#define XA 12.0
#define XB 34.0
#define XPLUS +
...
ZTEST(DevicetreeApi, test_string_idx_unquoted)
{
#define XA 12.0
#define XB 34.0
#define XC 56.0
...
#undef DT_DRV_COMPAT
#define DT_DRV_COMPAT vnd_adc_temp_sensor
ZTEST(DevicetreeApi, test_reset)
{
...
ZTEST(DevicetreeApi, test_interrupt_controller)
{
	/* DT_IRQ_INTC_BY_IDX */
